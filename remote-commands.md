...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > it's error because you don't configured git so new branchs on remote from local computer
...
6. Consider this situation:
   ...
   > Because you don't have the remote branch's code locally, an error will be raised.
    It is possible to fix this by using git fetch first, followed by git merge to combine your local code with the code you downloaded from github.
